...
import { avvicinaAForza } from '../src/engine/obiettivo-forza.js';
...
/**
 * Un mazzo di mostri: PS alti e danno alto, sopra la metà della scala 0–100.
 *
 * Vale 57, non 200: la scala di `forza.js` è assoluta e satura: offesa e
 * costanza sono già al massimo, ma `struttura` e `motore` restano a zero perché
 * il mazzo non ha linee evolutive e le Energie non coprono il fabbisogno. È
 * proprio quello che lo rende un buon caso di prova — gli scambi devono far
 * scendere il numero agendo sulle due voci che *non* sono già a fondo scala.
 */
const forzuto = () =>
  mazzo('Erba', ['Erba'], [
...
test('un obiettivo basso abbassa la forza senza cambiare la taglia del mazzo', () => {
  const m = forzuto();
  const partenza = forza(m).totale;
  const carteIniziali = m.totale;

  const esito = avvicinaAForza([m], { obiettivo: 45, dispensa: scorteScarse() });
  const arrivo = forza(m).totale;

  assert.ok(partenza > 50, `il mazzo di prova deve partire forte, era ${partenza}`);

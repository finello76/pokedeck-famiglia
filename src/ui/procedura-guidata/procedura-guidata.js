...
const DOMANDE = [
  ...
  },
  {
    ...
    // Senza catalogo **e** senza riferimento scelto la domanda non ha risposte:
    // sarebbe una schermata con un pulsante solo.
    mostraSe: (contesto) =>
      (contesto.prefatti?.length ?? 0) > 0 || contesto.forzaRiferimento != null,
    opzioni: (contesto) => [
      {
        valore: 'nessuno',
      ...
      // Il riferimento scelto in Impostazioni viene per primo — ma **solo** se
      // è un mazzo salvato. Se è un prefatto sta già nell'elenco qui sotto, e
      // aggiungerlo lo mostrerebbe due volte: due pulsanti diversi per la
      // stessa carta fanno pensare che facciano cose diverse.
      ...(contesto.forzaRiferimento != null && !contesto.idRiferimentoPrefatto
        ? [
            {
              valore: 'riferimento',
              etichetta: contesto.nomeRiferimento ?? 'Il mazzo di riferimento',
              dettaglio: `Il mazzo scelto in Impostazioni · forza ${contesto.forzaRiferimento}`,
      ...
      ...(contesto.prefatti ?? []).map((m) => ({
        valore: m.id,
        etichetta: m.nome,
        // Segnalato invece che ripetuto: chi ha già eletto questo Kit a metro
        // di casa lo ritrova al posto suo, con scritto che è lui.
        dettaglio: `${m.taglia} carte · forza ${m.forza}${
          m.id === contesto.idRiferimentoPrefatto ? ' · il tuo riferimento' : ''
        }`,
...
export function opzioniDaRisposte(risposte) {
  ...
  return {
    ...
    // e lascia i mazzi come li ha costruiti. È il secondo stadio della cascata,
    // quello degli scambi: il primo è `riferimento` qui sotto, che sceglie il
    // seme migliore prima ancora che ci sia qualcosa da scambiare.
